...
if (is_inter) {
  ...
  second_pred <- setdiff(inter_preds, "Condition")[1]
...
cont_inter2 <- emmeans(mod, specs = emmeans_formula, type = "response") |>
  contrast(interaction = "pairwise", by = NULL, adjust = "none", infer = TRUE)

cont_inter2@grid <- rename_with(cont_inter2@grid, \(c) stringr::str_remove(c, pattern = "_pairwise"))
```
...
paste0("- ", summary(cont_inter2) |> attr("mesg")) |> cat(sep = "\n")
```
