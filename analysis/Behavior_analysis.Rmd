...
contrasts(behavior_adults_Morris_data$Session) <- contr.sum
...
temporal_dist_plot(behavior_pups_data, "Time_GR")
```
...
```{r}
(behavior_pups_mod_GR_emm <- inner_join(
  emmeans::emmeans(
...
```{r}
(behavior_pups_mod_GR_contrasts <- emmeans::emmeans(
  object = behavior_pups_mod_GR, 
...
make_signif_boxplot(behavior_pups_mod_GR, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(behavior_pups_mod_GR)}] Boxplots - by [Condition]"), subfolder = behavior_target_pups, dpi = 600, width = 5, height = 8)
```

```{r fig.width = 6}
modeled_temporal_plot(
  behavior_pups_mod_GR, behavior_pups_mod_GR_emm, behavior_pups_mod_GR_contrasts, 
  time = "Stage", y_title = get_response_name("Time_GR")
) |> save_png(filename = glue("[{get_model_tag(behavior_pups_mod_GR)}] - Temporal evolution"), subfolder = behavior_target_pups, dpi = 600, width = 10, height = 8)
```
...
```{r}
distribution_summary(behavior_pups_data, dvs = c("Time_RR", "Time_RR_zi", "Time_RR_beta"), between = "Condition")
```

```{r}
purrr::map(
  c("Time_RR", "Time_RR_zi", "Time_RR_beta"),
  \(var) hist_plot(behavior_pups_data, var = var)
)
```
...
```{r fig.width = 6, fig.height = 3}
temporal_dist_plot(behavior_pups_data, "Time_RR")
```
...
behavior_pups_mod_RR_gamma_AR1 <- glmmTMB::glmmTMB(
  Time_RR ~ Condition * Stage + cs(Stage + 0 | Mouse),
  # disp = ~ Stage,
  family = Gamma("log"),
  data = behavior_pups_data,
  REML = T,
...
```{r}
(behavior_pups_mod_RR_emm <- inner_join(
  emmeans::emmeans(
...
```{r}
(behavior_pups_mod_RR_contrasts <- emmeans::emmeans(
  object = behavior_pups_mod_RR, 
  specs = ~ Condition | Stage,
  component = "cond",
  type = "response",
...
make_signif_boxplot(behavior_pups_mod_RR, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(behavior_pups_mod_RR)}] Boxplots - by [Condition]"), subfolder = behavior_target_pups, dpi = 600, width = 5, height = 8)
```

```{r fig.width = 8}
modeled_temporal_plot(
  behavior_pups_mod_RR, behavior_pups_mod_RR_emm, behavior_pups_mod_RR_contrasts, 
  time = "Stage", y_title = get_response_name("Time_RR"), show_observed = F
) |> save_png(filename = glue("[{get_model_tag(behavior_pups_mod_RR)}] - Temporal evolution"), subfolder = behavior_target_pups, dpi = 600, width = 10, height = 8)
```
...
make_signif_boxplot(behavior_teens_mod_Grip, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(behavior_teens_mod_Grip)}] Boxplots - by [Condition]"), subfolder = behavior_target_teens, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 6, fig.height = 3}
temporal_dist_plot(behavior_adults_Morris_data, "Path_Efficiency")
```

```{r fig.width = 6}
temporal_plot(behavior_adults_Morris_data, resp = "Path_Efficiency", pred = "Condition", time = "Session", y_name = get_response_name("Path_Efficiency"))
```
...
mod_behavior_adults_Morris_PE_GLMM_gauss <- glmmTMB(
  Path_Efficiency ~ Condition * Session + (1 | Mouse),
...
mod_behavior_adults_Morris_PE_AR1_beta <- glmmTMB(
  Path_Efficiency ~ Condition * Session + ar1(Session + 0 | Mouse),
  data = behavior_adults_Morris_data,
  family = beta_family("logit"),
  REML = T
)
...
ppc_plots(mod_behavior_adults_Morris_PE, mod_behavior_adults_Morris_PE_dharma_t, term = "Session")
```

```{r fig.width = 8}
ppc_stat_plots(mod_behavior_adults_Morris_PE, mod_behavior_adults_Morris_PE_dharma_t, term = "Condition")
ppc_stat_plots(mod_behavior_adults_Morris_PE, mod_behavior_adults_Morris_PE_dharma_t, term = "Session")
```
...
```{r}
(mod_behavior_adults_Morris_PE_emm <- inner_join(
  emmeans::emmeans(
    object = mod_behavior_adults_Morris_PE, 
    specs = ~ Session | Condition, 
    type = "response",
    lmer.df = "satterthwaite"
  ) |> as.data.frame() |> 
    select(Session, Condition, Modeled = matches("emmean|response"), matches("CL$")),

  distribution_summary(
    insight::get_data(mod_behavior_adults_Morris_PE), 
    dvs = insight::find_response(mod_behavior_adults_Morris_PE), 
    between = c("Condition", "Session")
  ) |> select(Session, Condition, Observed = Mean)
) |> select(Session, Condition, Observed, Modeled, everything()))
```

Contrasts:

```{r}
(mod_behavior_adults_Morris_PE_contrasts <- emmeans::emmeans(
  object = mod_behavior_adults_Morris_PE, 
  specs = ~ Condition | Session, 
  type = "response",
  adjust = "none"
) 
|> emmeans::contrast(method = "pairwise", type = "response", infer = TRUE) 
|> data.frame()
|> select(contrast, Session, matches("estimate|risk|odds|^ratio|^difference"), matches("CL$"), p.value)
)
```
...
make_signif_boxplot(mod_behavior_adults_Morris_PE, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_PE)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```

```{r fig.width = 6}
modeled_temporal_plot(mod_behavior_adults_Morris_PE, mod_behavior_adults_Morris_PE_emm, mod_behavior_adults_Morris_PE_contrasts, time = "Session") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_PE)}] Temporal evolution - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 10, height = 8)
```
...
```{r fig.width = 6, fig.height = 3}
temporal_dist_plot(behavior_adults_Morris_data, "Latency_Plat")
```

```{r fig.width = 6}
temporal_plot(behavior_adults_Morris_data, resp = "Latency_Plat", pred = "Condition", time = "Session", y_name = get_response_name("Latency_Plat"))
```
...
mod_behavior_adults_Morris_LP_GLMM_gauss <- glmmTMB(
  Latency_Plat ~ Condition * Session + (1 | Mouse),
  data = behavior_adults_Morris_data,
  family = gaussian("log"),
  REML = T
)
...
mod_behavior_adults_Morris_LP_AR1_gamma <- glmmTMB(
  Latency_Plat ~ Condition * Session + ar1(Session + 0 | Mouse),
  data = behavior_adults_Morris_data,
  family = Gamma("log"),
  REML = T
)
...
ppc_plots(mod_behavior_adults_Morris_LP, mod_behavior_adults_Morris_LP_dharma_t, term = "Session")
```

```{r fig.width = 8}
ppc_stat_plots(mod_behavior_adults_Morris_LP, mod_behavior_adults_Morris_LP_dharma_t, term = "Condition")
ppc_stat_plots(mod_behavior_adults_Morris_LP, mod_behavior_adults_Morris_LP_dharma_t, term = "Session")
```
...
```{r}
(mod_behavior_adults_Morris_LP_emm <- inner_join(
  emmeans::emmeans(
    ...
    select(Session, Condition, Modeled = response, matches("CL$")),
...
```{r}
(mod_behavior_adults_Morris_LP_contrasts <- emmeans::emmeans(
  object = mod_behavior_adults_Morris_LP, 
  specs = ~ Condition | Session, 
  type = "response",
  adjust = "none"
...
make_signif_boxplot(mod_behavior_adults_Morris_LP, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_LP)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```

```{r fig.width = 6}
modeled_temporal_plot(mod_behavior_adults_Morris_LP, mod_behavior_adults_Morris_LP_emm, mod_behavior_adults_Morris_LP_contrasts, time = "Session") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_LP)}] Temporal evolution - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 10, height = 8)
```
...
```{r fig.width = 6, fig.height = 3}
temporal_dist_plot(behavior_adults_Morris_data, "Freezing")
```

```{r fig.width = 6}
temporal_plot(behavior_adults_Morris_data, resp = "Freezing", pred = "Condition", time = "Day", y_name = get_response_name("Freezing"))
```
...
mod_behavior_adults_Morris_Freezing_GLMM_gauss <- glmmTMB(
  Freezing ~ Condition * Session + (1 | Mouse),
  data = behavior_adults_Morris_data,
  family = gaussian("log"),
  REML = T
)
...
mod_behavior_adults_Morris_Freezing_AR1_gamma <- glmmTMB(
  Freezing ~ Condition * Session + ar1(Session + 0 | Mouse),
  data = behavior_adults_Morris_data,
  family = Gamma("log"),
  REML = T
)
...
ppc_plots(mod_behavior_adults_Morris_Freezing, mod_behavior_adults_Morris_Freezing_dharma_t, term = "Session")
```

```{r fig.width = 8}
ppc_stat_plots(mod_behavior_adults_Morris_Freezing, mod_behavior_adults_Morris_Freezing_dharma_t, term = "Condition")
ppc_stat_plots(mod_behavior_adults_Morris_Freezing, mod_behavior_adults_Morris_Freezing_dharma_t, term = "Session")
```
...
```{r}
(mod_behavior_adults_Morris_Freezing_emm <- inner_join(
  emmeans::emmeans(
    ...
    select(Session, Condition, Modeled = matches("response|emmean"), matches("CL$")),
...
```{r}
(mod_behavior_adults_Morris_Freezing_contrasts <- emmeans::emmeans(
  object = mod_behavior_adults_Morris_Freezing, 
  specs = ~ Condition | Session, 
  type = "response",
  adjust = "none"
...
make_signif_boxplot(mod_behavior_adults_Morris_Freezing, xaxis = "Condition") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_Freezing)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```

```{r fig.width = 6}
modeled_temporal_plot(mod_behavior_adults_Morris_Freezing, mod_behavior_adults_Morris_Freezing_emm, mod_behavior_adults_Morris_Freezing_contrasts, time = "Session") |> 
  save_png(filename = glue("[{get_model_tag(mod_behavior_adults_Morris_Freezing)}] Temporal evolution - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 10, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
(make_signif_boxplot(behavior_adults_mod_PDT, xaxis = "Condition", cluster = NULL) + scale_y_continuous(labels = scales::percent)) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_PDT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
(make_signif_boxplot(behavior_adults_mod_PTT, xaxis = "Condition", cluster = NULL) + scale_y_continuous(labels = scales::percent))|> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_PTT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_AMS, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_AMS)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_AD, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_AD)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_CT, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_CT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
parameters::parameters(behavior_adults_mod_NM_gauss)
cat("\n")
performance::performance(behavior_adults_mod_NM_gauss)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_NM, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_NM)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_ORT, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_ORT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_OGT, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_OGT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_POT, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_POT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_PCT, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_PCT)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_NEO, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_NEO)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
...
```{r fig.width = 2, fig.height = 3}
make_signif_boxplot(behavior_adults_mod_NEC, xaxis = "Condition", cluster = NULL) |> 
  save_png(filename = glue("[{get_model_tag(behavior_adults_mod_NEC)}] Boxplots - by [Condition]"), subfolder = behavior_target_adults, dpi = 600, width = 5, height = 8)
```
